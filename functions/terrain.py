'''
Original code pulled from wofs
https://github.com/GeoscienceAustralia/wofs/blob/29f80e5d9a51fe95a79a3a0dfd896cf6a5403cb5/wofs/terrain.py#L86
'''
...
def solar_vector(point, time, crs):
    ...
    observer.date = time.values #### added .values in here 19.1.24 ####
...
def shadows_and_slope(tile, time, elevation_var='elevation', no_data=-1000):
    """
    ...
    Input: string - elevation_var is from Digital Surface Model xarray DataSet (i.e. 'dem_h')
    ...
    19.1.24 - Chris Owers changed 'elevation' throughout to be elevation_var that is now an input due to the elevation
    variable not necessarily being called 'elevation'
    """

    elevation = tile[elevation_var]
    ...
    # gradient and slope
    xgrad = ndimage.sobel(elevation, axis=1) / abs(8 * tile.affine.a)
    ygrad = ndimage.sobel(elevation, axis=0) / abs(8 * tile.affine.e)
    ...
    buff_elv_array = numpy.pad(elevation.values, 4, mode='edge')
    ...
    shadows = xarray.DataArray(shadows.reshape(elevation.shape), coords=elevation.coords)
